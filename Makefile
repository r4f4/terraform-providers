...
PROVIDER_TARGETS:=$(foreach DIR,$(SUBDIRS), bin/$(TARGET_OS_ARCH)/terraform-provider-$(DIR))
ZIP_TARGETS:=$(foreach PROV,$(PROVIDER_TARGETS), $(subst $(PROV),$(PROV).zip,$(PROV)))
...
$(PROVIDER_TARGETS): bin/$(TARGET_OS_ARCH)/terraform-provider-%: %/go.mod
	cd $*; \
...
$(ZIP_TARGETS): bin/$(TARGET_OS_ARCH)/terraform-provider-%.zip: bin/$(TARGET_OS_ARCH)/terraform-provider-%
